...
namespace SilverSim.Database.MySQL.UserAccounts
{
    ...
    public sealed class MySQLUserAccountService : UserAccountServiceInterface, IDBServiceInterface, IPlugin, IUserAccountSerialNoInterface
    {
        ...
        public override bool ContainsKey(string email)
        {
            ...
            {
                connection.Open();
                using (var cmd = new MySqlCommand("SELECT NULL FROM useraccounts WHERE Email = @email LIMIT 1", connection))
                {
        ...
        public override bool TryGetValue(string email, out UserAccount account)
        {
            ...
            {
                connection.Open();
                using (var cmd = new MySqlCommand("SELECT * FROM useraccounts WHERE Email = @email LIMIT 1", connection))
                {
                    cmd.Parameters.AddParameter("@email", email);
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
        ...
        public override bool ContainsKey(string firstName, string lastName)
        {
            ...
            {
                connection.Open();
                using (var cmd = new MySqlCommand("SELECT NULL FROM useraccounts WHERE FirstName = @firstname AND LastName = @lastname LIMIT 1", connection))
                {
        ...
        public override void Remove(UUID accountID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new MySqlCommand("DELETE FROM useraccounts WHERE ID = @id", connection))
                {
                    cmd.Parameters.AddParameter("@id", accountID);
        ...
        public override void SetEverLoggedIn(UUID accountID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new MySqlCommand("UPDATE useraccounts SET IsEverLoggedIn=1 WHERE ID = @id", connection))
                {
                    cmd.Parameters.AddParameter("@id", accountID);
                    if (cmd.ExecuteNonQuery() < 1)
                    {
