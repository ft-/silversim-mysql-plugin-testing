...
{
    public sealed partial class MySQLProfileService : IUserPreferencesInterface
    {
        bool IUserPreferencesInterface.ContainsKey(UGUI user)
        {
            ...
            {
                conn.Open();
                using (var cmd = new MySqlCommand("SELECT useruuid FROM usersettings where useruuid = @uuid LIMIT 1", conn))
                {
        ...
        bool IUserPreferencesInterface.TryGetValue(UGUI user, out ProfilePreferences prefs)
        {
            ...
            {
                conn.Open();
                using (var cmd = new MySqlCommand("SELECT * FROM usersettings where useruuid = @uuid LIMIT 1", conn))
                {
                    cmd.Parameters.AddParameter("@uuid", user.ID);
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
